    // 흰색 배경 위에서의 환상적인 시각적 효과를 위해 만들어진 raylib 색상 팔레트
    #define LIGHTGRAY  (Color){ 200, 200, 200, 255 }   // 밝은 회색
    #define GRAY       (Color){ 130, 130, 130, 255 }   // 회색
    #define DARKGRAY   (Color){ 80, 80, 80, 255 }      // 진한 회색
    #define YELLOW     (Color){ 253, 249, 0, 255 }     // 노랑색
    #define GOLD       (Color){ 255, 203, 0, 255 }     // 금색
    #define ORANGE     (Color){ 255, 161, 0, 255 }     // 오렌지색
    #define PINK       (Color){ 255, 109, 194, 255 }   // 핑크색
    #define RED        (Color){ 230, 41, 55, 255 }     // 빨간색
    #define MAROON     (Color){ 190, 33, 55, 255 }     // 밤색
    #define GREEN      (Color){ 0, 228, 48, 255 }      // 초록색
    #define LIME       (Color){ 0, 158, 47, 255 }      // 초록색
    #define DARKGREEN  (Color){ 0, 117, 44, 255 }      // 진한 초록색
    #define SKYBLUE    (Color){ 102, 191, 255, 255 }   // 하늘색
    #define BLUE       (Color){ 0, 121, 241, 255 }     // 파란색
    #define DARKBLUE   (Color){ 0, 82, 172, 255 }      // 진한 파란색
    #define PURPLE     (Color){ 200, 122, 255, 255 }   // 보라색
    #define VIOLET     (Color){ 135, 60, 190, 255 }    // 남보라색
    #define DARKPURPLE (Color){ 112, 31, 126, 255 }    // 진한 보라색
    #define BEIGE      (Color){ 211, 176, 131, 255 }   // 베이지색
    #define BROWN      (Color){ 127, 106, 79, 255 }    // 갈색
    #define DARKBROWN  (Color){ 76, 63, 47, 255 }      // 진한 갈색

    #define WHITE      (Color){ 255, 255, 255, 255 }   // 흰색
    #define BLACK      (Color){ 0, 0, 0, 255 }         // 검정색
    #define BLANK      (Color){ 0, 0, 0, 0 }           // 검정색 (투명)
    #define MAGENTA    (Color){ 255, 0, 255, 255 }     // 자홍색
    #define RAYWHITE   (Color){ 245, 245, 245, 255 }   // raylib 전용 색상 (raylib 로고)
