    ...
    void SetWindowState(unsigned int flags);                    //使用标志设置窗口配置状态(仅限PLATFORM_DESKTOP)
    ...
    void BeginTextureMode(RenderTexture2D target);              //开始绘制以渲染Texture
    ...
    //VR模拟器的VR立体配置功能
    VrStereoConfig LoadVrStereoConfig(VrDeviceInfo device);     //为VR模拟器设备参数加载VR立体声配置
    ...
    //屏幕空间相关功能
    Ray GetScreenToWorldRay(Vector2 position, Camera camera);           //从鼠标位置获取光线跟踪
    ...
    Vector2 GetWorldToScreenEx(Vector3 position,  Camera camera, int width, int height); //获取三维世界空间位置的大小位置
    Vector2 GetWorldToScreen2D(Vector2 position, Camera2D camera);      //获取2d相机世界空间位置的屏幕空间位置
    Vector2 GetScreenToWorld2D(Vector2 position, Camera2D camera);      //获取2d相机屏幕空间位置的世界空间位置
    ...
    void WaitTime(double seconds);              //等待一段时间(停止程序执行)
    
    //其他。功能
    void SetRandomSeed(unsigned int seed);      //设置随机数生成器的种子
    int GetRandomValue(int min, int max);       //获取最小值和最大值之间的随机值(包括两者)
    ...
    void SetConfigFlags(unsigned int flags);    //设置初始化配置标志(查看flags)
    ...
    void SetTraceLogCallback(TraceLogCallback callback); //设置自定义跟踪日志
    // Memory management, using internal allocators
    void *MemAlloc(unsigned int size);          //内部内存分配器
    void *MemRealloc(void *ptr, unsigned int size); //内部内存重新定位程序
    void MemFree(void *ptr);                    //内部内存可用

    //文件管理功能
    unsigned char *LoadFileData(const char *fileName, int *dataSize); //将文件数据加载为字节数组(读取)
    void UnloadFileData(unsigned char *data);                     //卸载LoadFileData()分配的文件数据
    bool SaveFileData(const char *fileName, void *data, int dataSize); //将数据从字节数组保存到文件(写入), 成功时返回true
    bool ExportDataAsCode(const unsigned char *data, int dataSize, const char *fileName); //将数据导出到代码(.h), 成功时返回true
    char*LoadFileText(const char *fileName);                      //从文件加载文本数据(读取), 返回以“\0”结尾的字符串
    void UnloadFileText(char *text);                              //卸载LoadFileText()分配的文件文本数据
    bool SaveFileText(const char *fileName, const char *text);    //将文本数据保存到文件(写入), 字符串必须以“\0”结尾, 成功时返回true

    // File access custom callbacks
    // WARNING: Callbacks setup is intended for advanced users
    void SetLoadFileDataCallback(LoadFileDataCallback callback);  //设置自定义文件二进制数据加载器
    void SetSaveFileDataCallback(SaveFileDataCallback callback);  //设置自定义文件二进制数据保护程序
    void SetLoadFileTextCallback(LoadFileTextCallback callback);  //设置自定义文件文本数据加载器
    void SetSaveFileTextCallback(SaveFileTextCallback callback);  //设置自定义文件文本数据保护程序
    ...
    bool ChangeDirectory(const char *dirPath);                    //更改工作目录, 成功时返回true
    ...
    FilePathList LoadDirectoryFilesEx(const char *basePath, const char *filter, bool scanSubdirs); //使用扩展筛选和递归目录扫描加载目录文件路径
    ...
    unsigned char *DecodeDataBase64(const char *text, int *outputSize);                            //解码Base64字符串数据, 内存必须为MemFree()
    ...
    bool IsGamepadButtonPressed(int gamepad, int button);   //检查游戏机按钮是否已按下一次
    bool IsGamepadButtonDown(int gamepad, int button);      //检查是否按下了游戏机按钮
    bool IsGamepadButtonReleased(int gamepad, int button);  //检查游戏机按钮是否已释放一次
    bool IsGamepadButtonUp(int gamepad, int button);        //检查是否未按下游戏机按钮
    int GetGamepadButtonPressed(void);                      //按下最后一个游戏机按钮
    int GetGamepadAxisCount(int gamepad);                   //获取游戏机的游戏机轴计数
    float GetGamepadAxisMovement(int gamepad, int axis);    //获取游戏机轴的轴移动值
    int SetGamepadMappings(const char *mappings);           //设置内部游戏机映射(SDL_GameControllerDB)
    ...
    //输入相关功能：鼠标
    bool IsMouseButtonPressed(int button);                  //检查是否按下了一次鼠标按钮
    bool IsMouseButtonDown(int button);                     //检查是否按下了鼠标按钮
    bool IsMouseButtonReleased(int button);                 //检查鼠标按钮是否已释放一次
    bool IsMouseButtonUp(int button);                       //检查是否未按下鼠标按钮
    ...
    void SetMouseCursor(int cursor);                        //设置鼠标光标，用于更改屏幕上的光标图像(如：当鼠标放在文本上时会变成另一种图案)
    ...
    //手势和触摸处理功能(模块：手势)
    //------------------------------------------------------------------------------------
    void SetGesturesEnabled(unsigned int flags);           //使用标志启用一组手势
    bool IsGestureDetected(unsigned int gesture);          //检查是否检测到手势
   ...
   //------------------------------------------------------------------------------------
    ...
    void UpdateCameraPro(Camera* camera, Vector3 movement, Vector3 rotation, float zoom); //更新相机移动/旋转
